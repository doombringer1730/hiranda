...
${suffix}`;
  ...
  // src/content/base.js
  var DRIFT_THRESHOLD = 1e3;
  var DRIFT_THRESHOLD_YT = 2500;
  var NTP_SAMPLE_MAX = 5;
  var HEARTBEAT_MS = 4e3;
  var NTP_INTERVAL_MS = 3e4;
  var SUPABASE_URL = "https://nqmawsssiutarjylnmhg.supabase.co";
  var SUPABASE_ANON = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5xbWF3c3NzaXV0YXJqeWxubWhnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkwMDYzODQsImV4cCI6MjA5NDU4MjM4NH0.MtuvqyUxeztP1scGTrebMKBaA6JjC2b0P1qARKmgPZw";
  var HIRANDA_URL = "https://hiranda-616i.vercel.app";
  ...
  var PLATFORM = host.includes("netflix.com") ? "netflix" : host.includes("youtube.com") ? "youtube" : host.includes("disneyplus.com") ? "disney" : host.includes("amazon.com") || host.includes("primevideo.com") ? "amazon" : host.includes("hiranda-616i.vercel.app") ? "hiranda" : null;
  if (!PLATFORM) {
  }
  var On = 0;
  var Pn = 0;
  ...
  var ntpTimer = null;
  var localListenersAttached = false;
  var moRef = null;
  var hasReceivedFirstSync = false;
  ...
  async function syncClock() {
    try {
      ...
      const now = Date.now();
      vn(sn, now - sentAt, NTP_SAMPLE_MAX);
      rn = gn(sn);
      vn(an, now - Math.round(rn / 2) - serverTime, NTP_SAMPLE_MAX);
      un = gn(an);
    } catch (e) {
    ...
    youtube: {
      ...
      get video() {
        ...
        if (href.includes("/shorts/"))
          return document.querySelector("#shorts-player .video-stream.html5-main-video") || document.querySelector("#shorts-player video");
        return null;
      },
      // Synchronous reads — no round-trip to MAIN world
      getCurrentTime() {
        return (this.video?.currentTime ?? 0) * 1e3;
      },
      isPaused() {
      ...
      isLoading() {
        const v = this.video;
        return !!v && !v.paused && v.readyState < 4;
      },
      // Wait for video to finish loading — Teleparty's waitVideoDoneLoadingAsync()
      async waitForLoaded(timeoutMs = 5e3) {
        const deadline = Date.now() + timeoutMs;
        while (Date.now() < deadline) {
          const v = this.video;
          if (v && (v.paused || v.readyState >= 4))
            return;
      ...
      async seek(ms) {
        window.dispatchEvent(new CustomEvent("YoutubeVideoMessage", {
          detail: { type: "seekTo", seekTo: ms / 1e3 }
        }));
        await new Promise((r) => setTimeout(r, 500));
        await this.waitForLoaded(5e3);
      },
      play() {
        window.dispatchEvent(new CustomEvent("YoutubeVideoMessage", { detail: { type: "playVideo" } }));
      },
      pause() {
        window.dispatchEvent(new CustomEvent("YoutubeVideoMessage", { detail: { type: "pauseVideo" } }));
      }
    },
  ...
  var A = PLATFORM === "netflix" ? adapter.netflix : PLATFORM === "youtube" ? adapter.youtube : adapter.standard;
  ...
  async function bn(sessionData, force = false) {
    ...
    if (!force && localState === sessionData.state && drift < effectiveDrift)
      return actions;
  ...
  var _applyQueue = Promise.resolve();
    ...
    _applyQueue = _applyQueue.then(async () => {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent("HirandaSyncAction", {
          detail: {
            event: sessionData.event || "heartbeat",
            // 'pause'|'play'|'seek'|'heartbeat'
            position: seekTarget,
            // ms we synced to
            actions
            // ['SEEK','PAUSE'] etc.
        ...
        if (incremented)
          uiEventsHappening = Math.max(0, uiEventsHappening - 1);
      }
    });
  }
  async function broadcast(force = false, triggeredAt = null, eventType = "heartbeat") {
    ...
    try {
      ...
      const payload = {
        ...
        // video position in ms at moment of action
        triggeredAt: actionAt,
        // server-normalised UTC wall-clock ms of the action
  ...
  function attachVideoEvents(vid) {
    ...
    vid.addEventListener("play", () => {
      if (guard())
        return;
      broadcast(false, Date.now() - un, "play");
    });
    vid.addEventListener("pause", () => {
      if (guard())
        return;
      broadcast(false, Date.now() - un, "pause");
    });
    vid.addEventListener("seeked", () => {
      if (guard())
        return;
      broadcast(false, Date.now() - un, "seek");
    });
  }
  function attachLocalListeners() {
    if (localListenersAttached)
      return;
    localListenersAttached = true;
    if (PLATFORM === "netflix") {
      window.addEventListener("FromNode", (e) => {
        if (e.detail?.type !== "UpdateState")
          return;
        if (uiEventsHappening > 0 || !isHost || !channel)
          return;
      ...
      attachVideoEvents(getVid());
      window.addEventListener("FromNode", (e) => {
        if (e.detail?.type !== "SpaNavigated")
          return;
        const vid = getVid();
        if (vid)
          delete vid._hirandaListeners;
        attachVideoEvents(getVid());
        if (isHost && channel)
          setTimeout(() => broadcast(), 1e3);
      });
    } else {
      const getVid = () => document.querySelector("video");
      moRef = new MutationObserver(() => {
  ...
  async function joinParty(id, host2 = false) {
    if (channel)
      channel.unsubscribe();
    partyId = id;
    isHost = host2;
  ...
  function checkUrlHash() {
    ...
    if (id && !partyId) {
      setTimeout(() => joinParty(id, false), 2e3);
    }
  }
  if (PLATFORM === "netflix")
    injectNetflixScript();
  ...
  (async () => {
    if (partyId)
      return;
    const state = await new Promise((r) => chrome.runtime.sendMessage({ type: "GET_PARTY_STATE" }, r));
