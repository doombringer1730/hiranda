// MAIN world — Netflix injected script
// Mirrors Teleparty's netflix_injected_bundled.js exactly
// Injected via web_accessible_resources + script tag

;(function () {
  'use strict'

  // ── React internals helpers (exact from Teleparty) ──────────────────────────
  ...
  // ── Netflix internal player API (exact from Teleparty) ──────────────────────
  ...
  const getPlayerApi = () => {
    try {
      ...
      return (
        fiber.return.return.return.return.return.stateNode ||
        fiber.return.return.return.return.stateNode
      )
    } catch { return undefined }
  }
  const isMovie = () => {
    try { return getWrapperStateNode().state.playableData.summary.type === 'movie' }
    catch { return false }
  }
  ...
  const getAdState = () => {
    ...
    } catch { return { watchingAds: false, adDurationLeft: 0 } }
  }

  // ── Player wrapper / controls show (from Teleparty) ─────────────────────────
  const delay = ms => new Promise(r => setTimeout(r, ms))
  const getPlayerWrapper = () => {
    const selectors = [
      'div[data-uia="player"]', 'div[data-videoid]',
  ...
  const changeEpisodeFallback = async id => {
    try {
      ...
      api.handleSelectorEpisodePlay({ stopPropagation: () => {} }, id)
  ...
  // ── postPlay disable (Teleparty does this for movies) ───────────────────────
  ...
  const teardownFixPostPlay = () => {
    const node = getWrapperStateNode()
    if (node && window.oldHasPostPlay) node.hasPostPlay = window.oldHasPostPlay
  }

  // ── Main message listener — exact from Teleparty's seekInteraction ───────────
    ...
    try {
      ...
      if (type === 'SEEK') {
        if (time >= player.duration) {
          player.pause(); player.seek(player.duration - 100)
        } else {
          player.seek(time - 100)  // Teleparty's -100ms offset
        }
      } else if (type === 'PLAY') {
        player.play()
      } else if (type === 'PAUSE') {
