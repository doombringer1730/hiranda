// MAIN world — YouTube injected script
// Mirrors Teleparty's youtube_injected_bundled.js exactly
// Injected via manifest content_scripts (world: MAIN, run_at: document_idle)

;(function () {
  'use strict'

  // ── Helpers (exact from Teleparty) ──────────────────────────────────────────
  ...
  // Teleparty: separate API player (for commands) vs raw video element (for state)
  const getVideoElement = () => {
    const href = window.location.href
    if (!isPlayerPage()) return null
    if (href.includes('/watch'))   return document.querySelector('#movie_player')
    if (href.includes('/shorts/')) return document.querySelector('#shorts-player')
    return null
  }

  // Theater mode helpers (exact from Teleparty)
  const setTheater = () => {
    const theaterButton = document.querySelector('.ytp-size-button')
    if (theaterButton) {
      const inTheater = document.querySelector('ytd-watch-flexy[theater]')
      if (!inTheater) theaterButton.click()
      theaterButton.style.display = 'none'
    }
  }
  const disableTheater = () => {
    const theaterButton = document.querySelector('.ytp-size-button')
    if (theaterButton) theaterButton.style.display = ''
  }

  // Teleparty resize for fullscreen + chat visible
  ...
  if (!window.videoIdScriptLoaded) {
    ...
    // ── Command listener (from ISOLATED world) ──────────────────────────────
    window.addEventListener('YoutubeVideoMessage', event => {
      ...
      if (type === 'pauseVideo') {
        if (!window.resizeScriptReady && window._yt_player) resizeYoutube(window._yt_player)
        video?.pauseVideo()
        ...
        // Teleparty: seekTo(seconds, allowSeekAhead=true)
        if (video) video.seekTo(event.detail.seekTo, true)
        ...
        // Kept for compatibility — base.js now reads raw element instead
        if (video) {
          const t    = video.getCurrentTime() ?? 0
          const paused = video.getPlayerState?.() !== 1
          window.dispatchEvent(new CustomEvent('FromNode', {
            detail: { type: 'PlayerState', time: t * 1000, paused, updatedAt: Date.now() }
          }))
        }
        ...
        // Teleparty: use setSize()/setInternalSize() to trigger player resize
        resizeVisible = event.detail.visible
        if (video) {
          try { video.setSize() }        catch {}
          try { video.setInternalSize() } catch {}
        }

      } else if (type === 'jumpToNextEpisode') {
        // Navigate to another video in YouTube SPA
        const urlPath = `/watch?v=${event.detail.nextVideoId}`
        const navigationData = {
          endpoint: {
            commandMetadata: {
              webCommandMetadata: {
                url: urlPath, rootVe: 3832, webPageType: 'WEB_PAGE_TYPE_WATCH'
              },
    ...
    // ── SPA navigation — dispatch SpaNavigated to ISOLATED world ─────────────
    // YouTube never does a full page reload between videos. We must detect
    // navigation via yt-navigate-finish so base.js can re-sync state.
